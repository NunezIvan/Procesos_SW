...
public class registrar_contometro extends javax.swing.JFrame {
    ...
    float cuota_agua,cuota_desague,igv,cargo_fijo,mora;
    int mes;
    
    
    public registrar_contometro(encargado encargadoingresado, Periodo periodoingresado, int mesSeleccionado) {
        initComponents();
        ...
        // Continuar solo si existe consumo_general
        ...
        this.mora = Float.parseFloat(this.consumo_general.getMora());
    ...
    private void initComponents() {
        ...
        setMaximumSize(new java.awt.Dimension(309, 232));
        ...
        jLabel3.setText("Lectura Anterior:");

        jLabel4.setFont(new java.awt.Font("Roboto Light", 1, 14)); // NOI18N
        jLabel4.setText("Lectura Actual:");
        ...
        jComboBox2.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { " 201", " 202", " 301", " 302", " 401", " 402", " 501", " 502", " 601", " 602" }));
    ...
    private void confirmar_panelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_confirmar_panelMouseClicked
        String lectAnt_str = jTextField4.getText();
        String lectAct_Str = jTextField5.getText();
        ...
        try {
            ...
            float total_lectura = (lectAct - lectAnt) / 1000.0f;

            if (consumoExistente != null) {
                int opcion = JOptionPane.showConfirmDialog(this, 
                    "Ya existe un consumo registrado para el apartamento " + apart + ". ¿Desea modificarlo?",
                    "Modificar Consumo",
                ...
                agregarConsumo(consumo_apartamento);
    ...
    private void actualizarConsumoExistente(consumo_agua consumoExistente, float total_lectura) {
        consumoExistente.setLecturaAnterior(Integer.parseInt(jTextField4.getText()));
        consumoExistente.setLecturaActual(Integer.parseInt(jTextField5.getText()));
        consumoExistente.setApartamento(jComboBox2.getSelectedItem().toString().trim());

        consumoExistente.setPagoConsumo(total_lectura * this.cuota_agua);
        consumoExistente.setPagoDesague(total_lectura * this.cuota_desague);
        consumoExistente.setCargoFijo(this.cargo_fijo/10);
        consumoExistente.setMora(String.valueOf(this.mora));
        float total_IGV = (consumoExistente.getPagoConsumo() + consumoExistente.getPagoDesague() + consumoExistente.getCargoFijo()) * this.igv;
        consumoExistente.setIgvConsumo(total_IGV);
        float total_consumo_agua = consumoExistente.getPagoConsumo() + consumoExistente.getPagoDesague() + consumoExistente.getCargoFijo() + total_IGV - this.mora / 10;
        consumoExistente.setMontoTotal(total_consumo_agua);

        boolean actualizado = actualizarConsumo(consumoExistente);

        if (actualizado) {
            JOptionPane.showMessageDialog(this, "Consumo actualizado con éxito.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
            this.setVisible(false);
        } else {
            JOptionPane.showMessageDialog(this, "Error al actualizar el consumo.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
