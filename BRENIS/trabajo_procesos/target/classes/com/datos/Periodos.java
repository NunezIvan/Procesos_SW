...
public class Periodos {
    ...
    public static void deshabilitarPeriodos() {
        PreparedStatement ps;
        Connection con = conexion.getConexion();
        String sql = "UPDATE periodo SET disponible = 0";
        try {
            ps = con.prepareStatement(sql);
            ps.executeUpdate();
            System.out.println("Todos los periodos han sido deshabilitados.");
        } catch (SQLException e) {
            System.out.println("Error al deshabilitar los periodos: " + e.getMessage());
        } finally {
            try {
                con.close();
            } catch (SQLException e) {
